    ...
    useEffect(() => {
        const fetchMovieDetails = async () => {
            try {
                ...
                const d = data.find((movie) => movie.show.id === Number(id));
                setMovie(d);
                setSummary(d.show.summary.replace(/<[^>]*>?/gm, ''));
                if (d.show.premiered) {
                    setYear(d.show.premiered.split('-')[0]);
                }
                ...
                if (d.show._links) {
                    ...
                    if (d.show._links.previousepisode?.href) {
                        episodePromises.push(
                            fetch(d.show._links.previousepisode.href)
                                .then(res => res.json())
                        );
                    }
                    
                    if (d.show._links.nextepisode?.href) {
                        episodePromises.push(
                            fetch(d.show._links.nextepisode.href)
                                .then(res => res.json())
                        );
                    }
                    ...
                    const episodeInfo = {
                        previous: d.show._links.previousepisode ? episodeData[0] : null,
                        next: d.show._links.nextepisode ? 
                            episodeData[d.show._links.previousepisode ? 1 : 0] : null,
                        latest: d.show._links.previousepisode ? episodeData[0] : null
                    };
